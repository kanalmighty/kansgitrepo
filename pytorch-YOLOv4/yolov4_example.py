...
class YOLOLayer(nn.Module):
    ...
    def forward(self, x, targets=None):
        ...
        grid_x = torch.linspace(0, g_dim - 1, g_dim).repeat(g_dim, 1).repeat(bs * self.num_anchors, 1, 1).view(
            x.shape).type(FloatTensor)  # [16.3.13.13]  每行内容为0-12,共13行
        grid_y = torch.linspace(0, g_dim - 1, g_dim).repeat(g_dim, 1).t().repeat(bs * self.num_anchors, 1, 1).view(
            y.shape).type(FloatTensor)  # [16.3.13.13]  每列内容为0-12,共13列（因为使用转置T）
        ...
        if targets is not None:
            ...
            nGT, nCorrect, mask, conf_mask, tx, ty, tw, th, tconf, tcls = build_targets(pred_boxes.cpu().data,
                                                                                        # 在13x13尺度上的预测框  [16,3,13,13,4]
                                                                                        targets.cpu().data,
                                                                                        # 坐标被归一化后的真值框filled_labels[16,50,5] 值在0-1之间
                                                                                        scaled_anchors,
                                                                                        # 缩放到13x13尺度下的3个锚框
                                                                                        ...
                                                                                        # 数据集类别数  coco数据集80
                                                                                        g_dim,
                                                                                        # feature map相对于原图的缩放倍数13
                                                                                        self.ignore_thres,
                                                                                        # 阈值（用于判断  真值框 与 3个原始锚框的iou > 阈值）
