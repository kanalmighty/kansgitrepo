...
def rand_precalc_random(min, max, random_part):
 ...
 # fill_truth_detection(bboxes, self.cfg.boxes, self.cfg.classes, flip, pleft, ptop, swidth,sheight, self.cfg.w, self.cfg.h)
...
class Yolo_dataset(Dataset):
    def __init__(self, lable_path, cfg):
        ...
        for line in f.readlines():
            data = line.split(" ")
            truth[data[0]] = []#例子{'1.jpg':[]}
            for i in data[1:]:
                truth[data[0]].append([int(j) for j in i.split(',')])
            #循环结束以后<class 'dict'>: {'train2017\\000000558840.jpg': [[199, 200, 276, 270, 52], [325, 104, 358, 209, 39]...]}
        self.truth = truth
    ...
    def __getitem__(self, index):
        ...
        # img_path = os.path.join('D:\\datasets\\coco2017\\', img_path)
        img_path = os.path.join('D:\\PyCharmSpace\\pytorch-YOLOv4\\data\\test_data\\', img_path)
        ...
        for i in range(use_mixup + 1):
            ...
            dhue = rand_uniform_strong(-self.cfg.hue, self.cfg.hue)#random.random() * (max - min) + min
            dsat = rand_scale(self.cfg.saturation)
            dexp = rand_scale(self.cfg.exposure)
            # 首先生成一些随机偏移的坐标，分别代表左右上下


            # 在原始图片的宽度乘以1个系数以后的正负区间中产生一个随机数
            pleft = random.randint(-dw, dw)
            pright = random.randint(-dw, dw)

            # 在原始图片的高度度乘以1个系数以后的正负区间中产生一个随机数
            ...
            #模糊0 - disable, 1 - blur background, 2 - blur the whole image
            if (self.cfg.blur):
            ...
            if self.cfg.letter_box:
                img_ar = ow / oh #原始图片w/h比
                net_ar = self.cfg.w / self.cfg.h #配置文件中的w/h比
