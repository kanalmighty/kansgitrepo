...
class AutoAugment(object):
    ...
    def __call__(self, img):
        # print('applying %s' % self.policies[self.policy_index])
        img = apply_policy(img, self.policies[self.policy_index])
        return img

#operations['Invert'](img,0.1)
operations = {
...
def apply_policy(img, policy):
    if random.random() < policy[1]:
        img = operations[policy[0]](img, policy[2])#operations['Invert'](img,0.1)
